...
def get_mrr(pre, truth):
    """
    计算MRR
    :param pre: (B,K) TOP-K indices predicted by the model
    :param truth: (B,1) ground truth
    :return: MRR (torch.Tensor), shape: scalar tensor
    """
    targets = truth.view(-1, 1).expand_as(pre)
    hits = (targets == pre).nonzero(as_tuple=False)

    if len(hits) == 0:
        return torch.tensor(0.0, device=pre.device)  # 保持类型一致，避免返回 int

    ranks = hits[:, -1] + 1
    ranks = ranks.float()
    r_ranks = torch.reciprocal(ranks)
    mrr = torch.sum(r_ranks) / targets.size(0)
    return mrr
